...
public class iamUserController {
    ...
    @GetMapping public ResponseEntity<List<iamUser>> getAlliamUser(@RequestParam Integer id,@RequestParam String domain_name,@RequestParam(required = false) String search) {
        return new ResponseEntity<>(iamus.getAlliamUser(id,domain_name,search), HttpStatus.FOUND);
    }
