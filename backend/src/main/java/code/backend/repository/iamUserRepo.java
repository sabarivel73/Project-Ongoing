...
public interface iamUserRepo extends JpaRepository<iamUser,Integer> {
    ...
    @Query(value = "select id,name,domain_name,role from iam_user where (:id_value is null or id != :id_value) and domain_name like :domain_name_value and (:search_value is null or (name like :search_1_value or name like :search_2_value))",nativeQuery = true)
    List<Object[]> findAll(@Param("id_value") Integer id,@Param("domain_name_value") String domain_name,@Param("search_value") String search,@Param("search_1_value") String search_1,@Param("search_2_value") String search_2);
