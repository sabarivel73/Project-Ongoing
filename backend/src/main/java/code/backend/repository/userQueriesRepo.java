...
public interface userQueriesRepo extends JpaRepository<userQueries,Integer> {
    ...
    @Query(value = "select * from user_queries where sender_id = :iamUser_id_value and response is null order by timestamp",nativeQuery = true)
    List<userQueries> yetToResponse(@Param("iamUser_id_value") Integer iamUser_value);
    @Query(value = "select t2.id,t1.name,t2.query,t2.response,t2.timestamp from iam_user as t1 join (select * from user_queries where domain_name like :domain_name_value and response is null) as t2 on t1.id = t2.sender_id order by t2.timestamp",nativeQuery = true)
    List<userdomainQueriesResponse> yetToResponseRootUser(@Param("domain_name_value") String domain_name);
    ...
    @Query(value = "select t2.id,t1.name,t2.query,t2.response,t2.timestamp from iam_user as t1 join (select * from user_queries where domain_name like :domain_name_value and response is not null) as t2 on t1.id = t2.sender_id order by t2.timestamp desc",nativeQuery = true)
    List<userdomainQueriesResponse> respondedQueriesRootUser(@Param("domain_name_value") String domain_name);
}
